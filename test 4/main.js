...
scene.onCreate = function () {
  ...
  scene.getScreen().addSprite().setScale(sW).setTranslationY(-sH * 0.2).setColor("00000080");
  ...
  Tester = Cubes[Cubes.length-1].addMesh('Box.b3m')
    .setTranslation(43.703239, 35.226414, 49.007751)
    .setRotation(32.235046, 16.44582, 53.138741)
    .setScale(1.29025)
    .setName("Tester")
    .setType('solid')
    .setMaterial('Default');

    console.log(Tester.getGlobalMatrix())


  sss = scene.getScreen().addSprite().setScale(sW / 2).setTranslation(sW / 2, -sH / 2, 0).setColor("00ff0040");
  sss.onTouchEnd = function () {    
    Reparent(Tester, Stack[Stack.length-1])
  }
}
